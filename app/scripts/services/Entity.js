...
angular.module('qldarchApp').factory('Entity', function (Uris, $q, $http, Request, GraphHelper, Expression, $filter, toaster, $cacheFactory) {
  ...
  var setupPicturesAndTypes = function (entities) {
    ...
    // @todo: remove kludge
    // This is a complete kludge because the server is not automatically
    // setting
    ...
    return Expression.loadAllFull('qldarch:Photograph').then(function (expressions) {
      ...
      angular.forEach(expressions, function (expression) {
        var depictsUri = expression[Uris.QA_DEPICTS_BUILDING] || expression[Uris.QA_DEPICTS_ARCHITECT] || expression[Uris.QA_DEPICTS_FIRM];
        if (depictsUri) {
  ...
  var entity = {
      ...
      findByName: function (name, summary, type) {
        ...
        return Request.getIndex('entity', type, summary, true).then(function (nonDigitalThings) {
          // Create name labels for them so we can use them for our
          // search
          ...
          return setupPicturesAndTypes(results).then(function (entities) {
            entities = $filter('orderBy')(entities, function (entity) {
              // Sort by last name, first name (if its a thing
              // with a last name)
       ...
       * 
       * @param type
       *          type of entity e.g. qldarch:Architect
