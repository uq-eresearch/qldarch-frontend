...
angular.module('qldarchApp').factory('Structure', function(Entity, GraphHelper, Expression, Uris, $q, $filter, Relationship) {
  // Service logic
  ...
  var postProcess = function(structures) {
    ...
    angular.forEach(structures, function(structure) {
      ...
      if (angular.isDefined(structure.lat) && angular.isDefined(structure.lon)) {
        if (-44.902578 < structure.lat && structure.lat < -9.102097 && 104.765625 < structure.lon && structure.lon < 159.697266) {
    ...
    return Expression.findByBuildingUris(uris, 'qldarch:Photograph').then(function(pictures) {
      angular.forEach(pictures, function(picture) {
        angular.forEach(structures, function(structure) {
          ...
          // this fixes the issue with the preferred image being accidentally
          // overidden
