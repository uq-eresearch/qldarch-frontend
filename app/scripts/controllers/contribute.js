...
angular.module('angularApp')
    .controller('ContributeCtrl', function ($scope, ENV, vcRecaptchaService, $http, Uris, $state, toaster) {
         ...
         * Send the reCaptcha response to the server (backend)
         * And use some of the server side APIs to validate it
         * See https://developers.google.com/recaptcha/docs/verify
         */
            if ($scope.recaptchaResponse && msg) {
            	if (msg.hasOwnProperty('content') && msg.hasOwnProperty('firstname')
                		&& msg.hasOwnProperty('lastname') && msg.hasOwnProperty('from')) {
            		var c = msg.content;
            		var fn = msg.firstname;
            	    var ln = msg.lastname;
            	    var f = msg.from;
            		if (!((!c || !c.length || !/\S/.test(c)) || (!fn || !fn.length || !/\S/.test(fn))
            				|| (!ln || !ln.length || !/\S/.test(ln)) || (!f || !f.length || !/\S/.test(f)))) {
            			console.log('msg', msg);
            ...
            } else if (!msg.hasOwnProperty('content') || !msg.hasOwnProperty('firstname')
            		|| !msg.hasOwnProperty('lastname') || !msg.hasOwnProperty('from')) {
        		toaster.pop('error', 'Error occured.', 'Sorry, you must completely fill out the form and validate reCAPTCHA.');
            } else if (((!msg.content || !msg.content.length || !/\S/.test(msg.content)) || (!msg.firstname || !msg.firstname.length || !/\S/.test(msg.firstname))
