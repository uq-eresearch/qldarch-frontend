...
angular.module('qldarchApp').controller(
    'AdminMergeCtrl',
    function($scope, Entity, $http, Uris, toaster, $state) {

      $scope.merge = function() {
        // /ws/rest/entity/merge?intoResource=<intoResource>&fromResource=<fromResource>.
        $http.put(
            Uris.JSON_ROOT + 'entity/merge?intoResource=' + encodeURIComponent($scope.to.uri) + '&fromResource=' +
                encodeURIComponent($scope.from.uri)).then(function() {
