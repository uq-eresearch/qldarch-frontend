...
lore.ore.model.CompoundObject = Ext.extend(Ext.util.Observable, {
        ...
        this.aggregatedResourceStore = new Ext.data.JsonStore({
            ...
            fields: [{
                ...
                }, // aggregated resource URI
                {
                    name: 'title',
                    type: 'string'
                }, // aggregated resource title
                {
                    name: 'index',
                    type: 'int',
                    defaultValue: '1000'
                }, // for storing order information, large default value ensures resources are initially added at the end
                {
                    name: 'representsCO',
                    type: 'boolean',
                    defaultValue: false
                }, // indicates if this represents a nested Resource Map
                {
                    name: 'representsAnno',
                    type: 'boolean',
                    defaultValue: false
                }, // indicates if this represents an annotation
                {
                    name: 'representsBasic',
                    type: 'boolean',
                    defaultValue: false
                }, // indicates if this represents a Basic Object
                {
                    name: 'isPlaceholder',
                    type: 'boolean',
                    defaultValue: false
                }, // indicates if this resource has a generated URI, being a placeholder for some concept/thing
                {
                    name: 'properties'
                } // all other properties, key is property uri, value is array of Property objects
            ]
        });
        ...
        if (this.loadedContent) {
            ...
            this.loadedContent.where('<' + this.aggregationURI + '> ore:aggregates ?url')
                ...
                .each(function () {
                    ...
                    var resourceData = {
                        uri: resourceURL,
                        properties: new lore.ore.model.ResourceProperties()
                    };
    ...
    serialize: function (format) {
        ...
        try {
            if (format == 'rdfquery') {
                ...
                return Ext.util.JSON.encode(databank.dump({
                    format: 'application/json'
                }));
            } else {
                return databank.dump({
                    format: 'application/rdf+xml',
                    serialize: true
                });
            }
    ...
    isDirty: function () {
        ...
        if (isEmpty && (!lore.ore.ui.graphicalEditor ||
            (lore.ore.ui.graphicalEditor.coGraph && lore.ore.ui.graphicalEditor.coGraph.getDocument().getFigures().getSize() == 0))) {
            return false;
        } else {
        ...
        '</w:body></w:document>', {
            ...
            processHyperlink: function (url) {
                ...
                if (linkidx && linkidx != -1) {
                    ...
                    result += '<w:p><w:hyperlink r:id="' + linkid + '"><w:r><w:rPr><w:rStyle w:val="Hyperlink" /></w:rPr>' + "<w:t>" + lore.util.escapeHTML(url) + "</w:t>" + "</w:r></w:hyperlink></w:p>";
                } else {
                            ...
                            if (prop.prefix != "layout") {
                                ...
                                if (prop.value.toString().match("^http://") == "http://") {
                                    ...
                                    if (propR && propR.get('isPlaceholder')) {
                                        ...
                                        res += '<w:p><w:r><w:rPr><w:rStyle w:val="Strong" /></w:rPr><w:t>' + lore.util.escapeHTML(Ext.util.Format.capitalize(prop.name)) + ': </w:t></w:r><w:r><w:t>' + lore.util.escapeHTML(prop.title) + '</w:t></w:r></w:p>\n';
                                    } else {
                                        // TODO: add rel link
                                        res += '<w:p><w:r><w:rPr><w:rStyle w:val="Strong" /></w:rPr><w:t>' + lore.util.escapeHTML(Ext.util.Format.capitalize(prop.name)) + ': </w:t></w:r><w:r><w:t>' + lore.util.escapeHTML(prop.title) + '</w:t></w:r></w:p>\n';
                                    }
                                } else {
                                    res += '<w:p><w:r><w:rPr><w:rStyle w:val="Strong" /></w:rPr><w:t>' + lore.util.escapeHTML(Ext.util.Format.capitalize(prop.name)) + ': </w:t></w:r><w:r><w:t>' + this.processHtml(prop.value) + '</w:t></w:r></w:p>\n';
                                }
                            }
                        }
                    }
