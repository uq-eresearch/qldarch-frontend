...
lore.ore.ui.graph.EntityFigure = function(model) {
    ...
    this.editorWindow = new Ext.Window({ 
		...
		editField: function(){
			try {
				this.getComponent(0).setValue(this.propEditor.placeHolderText.dom.innerHTML);
				this.show(); 
				this.focus();
			} catch (e){
		...
		bbar: [
			'->',
			{
				...
				handler: function(btn, ev){
					try{
                        var val = this.editorWindow.getComponent(0).getRawValue();
                        this.placeHolderText.dom.innerHTML = val;
...
Ext.extend(lore.ore.ui.graph.EntityFigure, lore.draw2d.Node, {
    ...
    createHTMLElement : function() {  
        ...
        this.placeHolderText = Ext.get(textarea).createChild({
            tag: "span",
            cls: "orelink",
    ...
    populateContextMenu : function(menu){
          menu.add({
	            text: "Edit placeholder text",
	            icon: lore.constants.baseUrl + "skin/icons/ore/note_edit.png",
	            scope: this,
	            handler: function(evt){
	            	this.editorWindow.editField();
	            }
          });
          menu.add({
